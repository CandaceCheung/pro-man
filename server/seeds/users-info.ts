...
export async function seed(knex: Knex): Promise<void> {
    ...
    for (let i = 0; i < itemIDs.length; i++) { //81 items, 54 types
        ...
        insertArray = []
        insertArray.push(
            { item_id: itemIDs[i].id, type_id: typeIDs[2 + counter].id, start_date: format(new Date(), 'yyyy-MM-dd'), end_date: format(new Date(Date.now() + Math.floor(Math.random() * 1200000000)), 'yyyy-MM-dd'), color: getRandomColor()},
        )
