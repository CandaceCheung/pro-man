...
export async function seed(knex: Knex): Promise<void> {
    ...
    for (let i of projectIDs) {
        insertArray = [];
        for (let j = 1; j < 4; j++) {
            insertArray.push({ name: `Item Group ${j}`, project_id: i.id, is_deleted: false })
        }
    ...
    for (let i = 0; i < itemIDs.length; i++) { //81 items, 54 types
        ...
        insertArray = []
        insertArray.push(
            { item_id: itemIDs[i].id, type_id: typeIDs[2 + counter].id, start_date: new Date(new Date().toDateString()).getTime() - Math.ceil(Math.random() * 10) * 8.64e+7, end_date: new Date(new Date().toDateString()).getTime() + Math.ceil(Math.random() * 10) * 8.64e+7, color: getRandomColor()},
        )
