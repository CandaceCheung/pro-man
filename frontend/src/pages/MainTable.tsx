...
export function MainTable() {
  ...
  const [itemsOrdersState, setItemsOrdersState] = useState<
    Record<number, Array<number>>
  >({});
  const [typesOrdersState, setTypesOrdersState] = useState<
    Record<number, Array<number>>
  >({});
  const [itemGroupsCollapsedState, setItemGroupCollapsedState] = useState<
    boolean[]
  >([]);
  const [itemGroupsInputSelectState, setItemGroupsInputSelectState] = useState<
    boolean[]
  >([]);
  const [itemGroupsInputValueState, setItemGroupsInputValueState] = useState<
    string[]
  >([]);

  const [membersFullName, setMembersFullName] = useState<
    Record<number, MembersFullName>
  >({});
  const [personsColors, setPersonsColors] = useState<Record<number, string>>(
    {}
  );

  const [newItemInputSelected, setNewItemInputSelected] = useState<
  ...
  const [deleteGroupModalOpened, setDeleteGroupModalOpened] = useState<
    Record<number, boolean>
  >({});
  ...
  useEffect(() => {
          ...
          case "money":
            if (itemCells[itemGroupID][itemID][typeID]) {
              if (
                !itemCells[itemGroupID][itemID][
                  typeID
                ]!.transaction_id!.includes(cell.transaction_id)
              ) {
                itemCells[itemGroupID][itemID][typeID]!.transaction_id!.push(
                  cell.transaction_id
                );
                itemCells[itemGroupID][itemID][
                  typeID
                ]!.item_money_cashflow!.push(cell.item_money_cashflow);
                itemCells[itemGroupID][itemID][typeID]!.item_money_date!.push(
                  cell.item_money_date
                );
              }
            } else {
          ...
          case "persons":
            if (itemCells[itemGroupID][itemID][typeID]) {
              if (
                !itemCells[itemGroupID][itemID][
                  typeID
                ].item_person_user_id!.includes(cell.item_person_user_id)
              ) {
                itemCells[itemGroupID][itemID][
                  typeID
                ].item_person_user_id!.push(cell.item_person_user_id);
              }
            } else {
    ...
    personsMembersArray.forEach((id, index) => {
      personsColorsTemp[id] =
        theme.colors.personsTypeComponentColor[
          index % theme.colors.personsTypeComponentColor.length
        ];
    });
  ...
  const toggleItemGroupCollapsed = (index: number) => {
    const newItemGroupsCollapsedState = produce(
      itemGroupsCollapsedState,
      (draftState) => {
  ...
  const selectItemGroupInput = (index: number) => {
    const newItemGroupsInputSelectState = produce(
      itemGroupsInputSelectState,
      (draftState) => {
  ...
  const changeItemGroupInputValue = (index: number, value: string) => {
    const newItemGroupsInputValueState = produce(
      itemGroupsInputValueState,
      (draftState) => {
  ...
  const deselectItemGroupInput = (index: number) => {
    if (userId && projectID) {
      ...
      if (
        ...
        itemGroupsState[index].item_group_name
      ) {
        if (itemGroupsInputValueState[index].length) {
          // Set new group name into itemGroupsState
          const nextNewItemGroupsState = produce(
            itemGroupsState,
            (draftState) => {
              draftState[index].item_group_name =
                itemGroupsInputValueState[index];
            }
          );
  ...
  const handleDragEndRow = (event: any, item_group_id: number) => {
    ...
    if (active.id !== over.id && userId && projectID) {
      ...
      const nextItemOrdersState = produce(itemsOrdersState, (draftState) => {
        draftState[item_group_id] = arrayMove(
          draftState[item_group_id],
          oldIndex,
          newIndex
        );
      });
      ...
      userId &&
        dispatch(
          reorderItems(nextItemOrdersState[item_group_id], userId, projectID)
        );
    }
  };
  ...
  const handleDragEndColumn = (event: any, item_group_id: number) => {
    ...
    if (active.id !== over.id && userId && projectID) {
      ...
      userId &&
        dispatch(
          reorderTypes(nextTypesOrdersState[item_group_id], userId, projectID)
        );
    }
  };
    ...
    const newItemCellsState = produce(itemCellsState, (draftState) => {
      Object.keys(draftState).forEach((groupId, _) => {
        Object.keys(draftState[parseInt(groupId)]).forEach((itemId, _) => {
          Object.keys(draftState[parseInt(groupId)][parseInt(itemId)]).forEach(
            (each, _) => {
              if (parseInt(each) === typeId) {
                draftState[parseInt(groupId)][parseInt(itemId)][
                  typeId
                ].element_name = name;
              }
            }
  ...
  const onRemovePerson = (
    ...
    if (
      itemCellsState[groupId][itemId][typeId].item_person_user_id!.length <= 1
    ) {
      showNotification({
        title: "Delete person notification",
        message:
          "Failed to delete person! At least one person is required for items! 🤥",
      });
    } else {
      const newItemCellsState = produce(itemCellsState, (draftState) => {
        draftState[groupId][itemId][typeId].item_person_user_id = draftState[
          groupId
        ][itemId][typeId].item_person_user_id?.filter((id) => id !== personId);
      });
      setItemCellsState(newItemCellsState);
  ...
  const onAddTransaction = (
      ...
      const newItemCellsState = produce(itemCellsState, (draftState) => {
        for (const i in draftState[groupId][itemId][typeId].item_money_date!) {
          ...
          if (new Date(each) >= date) {
            draftState[groupId][itemId][typeId].transaction_id!.splice(
              parseInt(i),
              0,
              transactionId
            );
            draftState[groupId][itemId][typeId].item_money_cashflow!.splice(
              parseInt(i),
              0,
              cashFlow
            );
            draftState[groupId][itemId][typeId].item_money_date!.splice(
              parseInt(i),
              0,
              date.toISOString()
            );
        ...
        draftState[groupId][itemId][typeId].item_money_date!.push(
          date.toISOString()
        );
      });
      setItemCellsState(newItemCellsState);
    ...
    dispatch(
      addTransaction(itemId, format(date, "yyyy-MM-dd"), cashFlow, updateState)
    );
  };
  ...
  const onDeleteTransaction = (
    ...
    if (itemCellsState[groupId][itemId][typeId].transaction_id!.length <= 1) {
      showNotification({
        ...
        message:
          "Failed to delete transaction! At least one transaction is required for items! 🤥",
      });
    } else {
      const newItemCellsState = produce(itemCellsState, (draftState) => {
        const deleteIndex =
          draftState[groupId][itemId][typeId].transaction_id!.indexOf(
            transactionId
          );
        if (deleteIndex > -1) {
          draftState[groupId][itemId][typeId].transaction_id!.splice(
  ...
  const deselectNewItemNameInput = (groupId: number) => {
    if (newItemInputValue[groupId].length) {
      projectID &&
        userId &&
        dispatch(
          insertItem(projectID, userId, groupId, newItemInputValue[groupId])
        );
    }
  ...
  const onDeleteItem = (groupId: number, itemId: number) => {
    if (Object.keys(itemCellsState[groupId]).length <= 1) {
      showNotification({
        ...
        message:
          "Failed to delete item! Each group should have at least 1 item! 🤥",
      });
    } else {
      userId &&
        projectID &&
        dispatch(deleteItem(groupId, itemId, userId, projectID));
    }
  };
  ...
  const onDeleteGroup = (groupId: number) => {
    ...
    if (Object.keys(itemCellsState).length <= 1) {
      showNotification({
        title: "Item delete notification",
        message:
          "Failed to delete group! Each project should have at least 1 item group! 🤥",
      });
    } else {
      userId &&
        projectID &&
        dispatch(deleteItemGroup(groupId, userId, projectID));
    }
  };

  return (
    <ScrollArea
      style={{ width: "calc(100vw - 140px)", height: "calc(100vh - 160px)" }}
      type="auto"
    >
      <div className="main-table">
        {itemGroupsState.map(
          ({ item_group_id, item_group_name }, itemGroupArrayIndex) => {
            return (
              <div className={classes.itemGroup} key={itemGroupArrayIndex}>
                ...
                >
                  <span
                    ...
                    onClick={() =>
                      toggleDeleteGroupModalSelected(item_group_id)
                    }
                  >
                    <IconX size={16} />
                  </span>
                  ...
                  <span
                    onClick={() =>
                      toggleItemGroupCollapsed(itemGroupArrayIndex)
                    }
                  ...
                  <span className={classes.itemCount}>
                    {itemGroupsInputSelectState[itemGroupArrayIndex] ? (
                      <input
                        onBlur={() =>
                          deselectItemGroupInput(itemGroupArrayIndex)
                        }
                        type="text"
                        autoFocus
                      ...
                      <span
                        onClick={() =>
                          selectItemGroupInput(itemGroupArrayIndex)
                        }
                        className={cx(
                          classes.groupName,
                          classes.hovertext,
                        ...
                        item-count={
                          itemsOrdersState[item_group_id].length
                            ? itemsOrdersState[item_group_id].length +
                              " item" +
                              `${
                                itemsOrdersState[item_group_id].length === 1
                                  ? ""
                                  : "s"
                              }`
                            : "No items"
                    ...
                    >
                      <div className={classes.tableHead}>
                        <div className={classes.tableRow}>
                          ...
                          <DndContext
                            sensors={sensors}
                            onDragEnd={(event) =>
                              handleDragEndColumn(event, item_group_id)
                            }
                          >
                            <SortableContext
                            ...
                            >
                              {typesOrdersState[item_group_id].map(
                                (typeId, index) => (
                                  <TableColumnTitle
                                    key={typeId}
                                    id={typeId}
                                    ...
                                    lastCell={
                                      index ===
                                      typesOrdersState[item_group_id].length - 1
                                    }
                      ...
                      <div className={classes.tableBody}>
                        <DndContext
                          ...
                          onDragEnd={(event) =>
                            handleDragEndRow(event, item_group_id)
                          }
                        >
                          <SortableContext
                          ...
                          >
                            {itemsOrdersState[item_group_id].map(
                              (itemId, itemIndex) => (
                                <TableRow
                                  key={
                                    "group_" +
                                  ...
                                  cellDetails={
                                    itemCellsState[item_group_id][itemId]
                                  }
                                  color={
                                    theme.colors.groupTag[
                                      item_group_id %
                                        theme.colors.groupTag.length
                                    ]
                                  }
                                  lastRow={
                                    itemIndex ===
                    ...
                    <div className={classes.addItemCell}>
                      ...
                      <div className={cx(classes.tableCell, classes.item)}>
                        {newItemInputSelected[item_group_id] ? (
                          <input
                            onBlur={() =>
                              deselectNewItemNameInput(item_group_id)
                            }
                            type="text"
                            autoFocus
                            ...
                            onKeyDown={(e) =>
                              handleNewItemNameInputKeyDown(
                                e.key,
                                item_group_id
                              )
                            }
                            onChange={(e) =>
                              updateNewItemInputValue(
                                item_group_id,
                                e.target.value
                              )
                            }
                          ...
                          <div
                            ...
                            onClick={() =>
                              toggleNewItemInputSelected(item_group_id)
                            }
                          >
                            + Add Item
                          </div>
