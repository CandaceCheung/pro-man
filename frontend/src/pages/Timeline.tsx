...
export function TimeFrame() {
  ...
  const startPointAnchor = useAppSelector(
    (state) => state.project.time_line_start_anchor
  );
  const endPointAnchor = useAppSelector(
    (state) => state.project.time_line_end_anchor
  );
  ...
  const toggleUpdateModal = useAppSelector(
    (state) => state.project.update_time_line_modal_opened
  );
  const stack = useAppSelector((state) => state.project.time_line_stack_item);
  const sortByPersonId = useAppSelector(
    (state) => state.project.sort_by_person_id
  );
  const sortByGroupId = useAppSelector(
    (state) => state.project.sort_by_group_id
  );
  const setHideByType = useAppSelector(
    (state) => state.project.set_hide_by_type
  );
  const itemHeight = useAppSelector(
    (state) => state.project.set_timeline_item_height
  );
  ...
  const timelineDetail = unfilteredTimelineDetails
    .filter((project) =>
      sortByPersonId ? project.item_person_user_id === sortByPersonId : project
    )
    .filter((project) =>
      sortByGroupId ? project.item_group_id === sortByGroupId : project
    );
  ...
  for (let item of timelineDetail) {
    ...
    lastEndedTime = Math.max(
      new Date(item.item_dates_datetime).getTime() + 8.64e7,
      lastEndedTime
    );
  }
  ...
  for (let item of timelineDetail) {
    ...
    firstStartedTime = Math.min(
      new Date(item.item_dates_datetime).getTime(),
      firstStartedTime
    );
  }
  ...
  function handleItemResize(
    ...
    if (itemId.toString()[0] === "1") {
      ...
      const originalStartTime = items.filter((x) => x.id === itemId)[0]
        .start_time;
    ...
    <div id="timeline-container">
      {loading ? (
        ...
        <Timeline
          ...
          visibleTimeStart={
            autofit
              ? firstStartedTime - 2.592e8
              : now
              ? startPointAnchor
              : undefined
          }
          visibleTimeEnd={
            autofit ? lastEndedTime + 2.592e8 : now ? endPointAnchor : undefined
          }
          useResizeHandle
                  ...
                  {({ styles, date }) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          Start Date :{" "}
                          {new Date(firstStartedTime).toDateString()}
                        </span>
                        <span className="pin">
                          <IconPinned size={30} />
                        </span>
                  ...
                  {({ styles, date }) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className="pin-label"
                          style={{ background: "deepskyblue", right: "10px" }}
                        >
                  ...
                  {({ styles, date }) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className="pin-label"
                          style={{ background: "deeppink", left: "10px" }}
                        >
