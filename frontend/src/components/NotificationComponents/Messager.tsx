...
export function Messager() {
  ...
  const inviteMemberOpened = useAppSelector(
    (state) => state.project.toggle_invite_member_modal
  );
    ...
    timer = setTimeout(() => {
      if (pattern.test(username) || username.length < 1) {
        showNotification({
          ...
          message:
            "Input field is empty or contains invalid characters, please remove them and try again",
        });
        setLoading(false);
      } else {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (messageType === "invite") {
      ...
      dispatch(
        sendMessage(name!, userId!, username, targetUser!, text, messageType)
      );
    } else {
      showNotification({
        ...
        <form onSubmit={(e) => handleSubmit(e)}>
          <Input.Wrapper label="Receiver" required>
            <Input
              ...
              rightSection={
                <Tooltip
                  label="Wait 3 seconds after finishing input"
                  position="top-end"
                  withArrow
                >
                  {loading ? (
            ...
            <>
              <Input.Wrapper label="Select Message Type" required>
                <Input
                  ...
                  onChange={(e) =>
                    setMessageType(
                      e.target.value as "invite" | "message" | undefined
                    )
                  }
