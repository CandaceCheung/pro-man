...
import {
  setHideByTypeAction,
  setSortByGroupIdAction,
  setSortByPersonIdAction,
...
export function ButtonHub() {
  ...
  const groupSummary = projectSummary.filter(
    (project, index, self) =>
      ...
      index ===
        self.findIndex((obj) => obj.item_group_id === project.item_group_id)
  );
  const personsSummary = projectSummary.filter(
    (project, index, self) =>
      project.project_id === projectId && project.type_name === "persons"
  );
  ...
  const onSortByPersonsClick = (e: MouseEvent<HTMLButtonElement>) => {
    page === "timeline" &&
      dispatch(setSortByPersonIdAction(parseInt(e.currentTarget.value)));
  };
  const onSortByGroupClick = (e: MouseEvent<HTMLButtonElement>) => {
    page === "timeline" &&
      dispatch(setSortByGroupIdAction(parseInt(e.currentTarget.value)));
  };
