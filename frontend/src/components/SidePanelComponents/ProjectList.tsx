...
export function ProjectList() {
  ...
  const handleDeleteProject = (projectId: number) => {
    ...
    if (userId) {
      if (
        projectSummary.filter((each) => each.creator_id === userId).length <= 1
      ) {
        showNotification({
          title: "Delete project notification",
          message:
            "Failed to delete project! You must have at least 1 project! 🤥",
        });
      } else {
                ...
                >
                  ...
                  <span className="modal-footer">
                    <Button
                      ...
                      onClick={() =>
                        content.project_id &&
                        handleDeleteProject(content.project_id)
                      }
                    >
                      Delete
                    </Button>
