...
export function ProjectList (){
    ...
    let projectList :[{name: string, project_id: number, creatorId: number}?]  = []
            ...
            <div id="project-list">
                <Divider labelPosition='center' my="md" label="Your Projects" color={'dark'}/>
                    {projectList.map((content, index)=> content?.creatorId === userId &&
                        <div key={content.project_id}><Button onClick={(e)=>dispatch(setActiveProject(parseInt(e.currentTarget.value)))} value={content?.project_id} className='' variant='subtle' key={index}>{content?.name}</Button></div>
                    )}
