...
export default function ProjectNavbar() {
  ...
  const [projectTitleInputSelected, setProjectTitleInputSelected] =
    useState(false);
  ...
  function deselectProjectTitleInput() {
    if (projectTitleInputValue !== projectName) {
      if (projectTitleInputValue.length) {
        projectId &&
          dispatch(renameProject(projectId!, projectTitleInputValue));
      } else {
        projectName && setProjectTitleInputValue(projectName);
      }
    }
    ...
    <div id="project-navbar">
      <div className="navbar-header">
        <span id="project-title-container">
          ...
          <FontAwesomeIcon
            id="like-button"
            icon={like.length ? faStarS : faStarR}
            onClick={onLikeClick}
          />
        </span>
      ...
      <Tabs
        defaultValue={page}
        value={page}
        onTabChange={(value) => tabChangeHandler(value as ActivePageState)}
      >
        <Tabs.List>
