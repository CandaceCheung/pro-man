...
export function StatusColumn(props: StatusProps) {
    ...
    const addItem = () => {
        if (date !== undefined && groupId !== undefined) {
            ...
            dispatch(postItem(projectId, props.id, userId, itemName, nameList, memberId, date, groupId));
            setOpened(false);
        }
    };
    ...
    return (
        <div ref={setNodeRef} style={style} {...attributes}>
            <Card shadow='md' pt={1} pb={10} pr={6} pl={6} radius='md' withBorder mt={15} mr={6} w={290}>
                ...
                <>
                    <Modal opened={opened} onClose={() => setOpened(false)} title='Add Item' size='sm'>
                        <div>
                            {modalList.map((eachRow) => (
                                <Group className={classes.modalRow} key={eachRow.text}>
                                    <Text className={classes.modalText}>{eachRow.text}</Text>
                                    {eachRow.input}
                                </Group>
                            ))}
