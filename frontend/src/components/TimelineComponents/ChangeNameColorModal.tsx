...
export function ChangNameColorModal() {
  ...
  const targetElementId = useAppSelector(
    (state) => state.project.target_element_id
  );
  const itemType =
    targetElementId.toString()[0] === "0"
      ? null
      : targetElementId.toString()[0] === "1"
      ? "times"
      : "dates";
  ...
  const dateItem = projectSummary.filter(
    (project) =>
      project.item_datetime_id === itemId && project.type_name === "dates"
  )[0];
  const timeItem = projectSummary.filter(
    (project) =>
      project.item_times_id === itemId && project.type_name === "times"
  )[0];
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (itemType === "dates") {
      ...
      dispatch(
        updateDatelineItem(itemId, new Date(date).getTime(), name, color)
      );
    } else {
    ...
    <>
      <Modal opened={opened} onClose={() => onClose()} title="Update Item">
        <form onSubmit={handleSubmit}>
          <Input.Wrapper
            id={id}
            label={itemType === "times" ? "Time Block Name" : "Date Block Name"}
          >
            <Input
              type="text"
              value={name}
              ...
              rightSection={
                <Tooltip
                  label="This will be shown on the time block"
                  position="top-end"
                  withArrow
                >
                  <div>
