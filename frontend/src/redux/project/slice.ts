...
const setActiveProject: CaseReducer<
  ActiveProjectState,
...
const setTimelineNow: CaseReducer<
  ActiveProjectState,
...
const triggerWarningModal: CaseReducer<
  ActiveProjectState,
  PayloadAction<boolean>
> = (state, action) => {
  state.warning_modal_opened = action.payload;
};
const toggleMessager: CaseReducer<
  ActiveProjectState,
  PayloadAction<boolean>
> = (state, action) => {
  state.toggle_messager = action.payload;
};
const setActivePage: CaseReducer<
  ActiveProjectState,
  PayloadAction<ActivePageState | null>
> = (state, action) => {
  state.active_page = action.payload;
};
const toggleSidePanel: CaseReducer<
  ActiveProjectState,
  PayloadAction<boolean>
> = (state, action) => {
  state.toggle_side_panel = action.payload;
};
const toggleFavorite: CaseReducer<
  ActiveProjectState,
  PayloadAction<boolean>
> = (state, action) => {
  state.toggle_favorite = action.payload;
};
const triggerUpdateTimelineModal: CaseReducer<
  ActiveProjectState,
  PayloadAction<boolean>
> = (state, action) => {
  state.update_time_line_modal_opened = action.payload;
};
const setTargetUpdateElement: CaseReducer<
  ActiveProjectState,
  PayloadAction<number>
> = (state, action) => {
...
const toggleStackItem: CaseReducer<
  ActiveProjectState,
  PayloadAction<boolean>
> = (state, action) => {
  state.time_line_stack_item = action.payload;
};
const setSortByPersonId: CaseReducer<
  ActiveProjectState,
  PayloadAction<number>
> = (state, action) => {
  state.sort_by_person_id = action.payload;
};
const setSortByGroupId: CaseReducer<
  ActiveProjectState,
  PayloadAction<number>
> = (state, action) => {
...
const setTimelineItemHeight: CaseReducer<
  ActiveProjectState,
  PayloadAction<number>
> = (state, action) => {
  state.set_timeline_item_height = action.payload;
};
const toggleInvitationButton: CaseReducer<
  ActiveProjectState,
  PayloadAction<boolean>
> = (state, action) => {
  state.toggle_invitation_button = action.payload;
};
const toggleIReplyModal: CaseReducer<
  ActiveProjectState,
  PayloadAction<boolean>
> = (state, action) => {
  state.toggle_reply_modal = action.payload;
};
const toggleInviteMemberModal: CaseReducer<
  ActiveProjectState,
  PayloadAction<boolean>
> = (state, action) => {
...
const setMessageTarget: CaseReducer<
  ActiveProjectState,
  PayloadAction<number>
> = (state, action) => {
...
const sendMessage: CaseReducer<
  ActiveProjectState,
  PayloadAction<MessageState>
> = (state, action) => {
  state.message_summary.unshift(action.payload);
};
const getMessages: CaseReducer<
  ActiveProjectState,
  PayloadAction<MessageStateArr>
> = (state, action) => {
  state.message_summary = action.payload;
};
const toggleRead: CaseReducer<
  ActiveProjectState,
  PayloadAction<{ notificationId: number; checked: boolean }>
> = (state, action) => {
...
const toggleDelete: CaseReducer<
  ActiveProjectState,
  PayloadAction<{ notificationId: number; isDeleted: boolean }>
> = (state, action) => {
  for (let message of state.message_summary) {
    if (message.id === action.payload.notificationId) {
      message.status = true;
...
const getMemberList: CaseReducer<
  ActiveProjectState,
  PayloadAction<MyMemberStateArr>
> = (state, action) => {
  state.member_list = action.payload;
};
const changeAvatar: CaseReducer<
  ActiveProjectState,
  PayloadAction<{ membershipId: number[]; avatar: number }>
> = (state, action) => {
...
const deleteMember: CaseReducer<
  ActiveProjectState,
  PayloadAction<{ membershipId: number; projectId: number }>
> = (state, action) => {
